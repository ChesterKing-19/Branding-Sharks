import { type CSSProperties, type FC, useRef, useState, useEffect } from 'react';
...
class InfiniteGridMenu {
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown) {
      ...
      const pullBack = Math.min(this.control.rotationVelocity * 80 + 2.5, this.maxPullback);
      cameraTargetZ += pullBack;
      damping = 7 / timeScale;
    }
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (sketchRef.current) {
        sketchRef.current.maxPullback = mq.matches ? 2.5 : Infinity;
      }
    };
    apply();
  ...
  return (
    ...
    >
      ...
      {activeItem && (
        <>
          <h2
            className={`face-title ${activeItem.title.length > 14 ? 'long' : ''} ${
          ...
          <p className={`face-description ${isMoving ? 'inactive' : 'active'}`}>
            {isMobile && activeItem.shortDescription ? activeItem.shortDescription : activeItem.description}
          </p>
